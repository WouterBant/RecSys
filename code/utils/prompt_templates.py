...
def create_prompt_qa_fast(titles):
    ...
    for t in titles:
        ...
        prompt += f"{t}, "
    prompt += (
        "hvilken af følgende artikler er brugeren mest tilbøjelig til at klikke på?\n"
    )
    return prompt
...
def create_prompt_diversity(data):
    ...
    for t, c in zip(titles, categories):
        ...
        prompt += f"({c}) {t}, "
    prompt += (
        f"vil brugeren læse artiklen ({data['category']}) {data['title']}? (ja/nej)\n"
    )
    return prompt
...
def create_prompt_w_publishtime(data):
    def get_time_diff(t1, t2):
        ...
        if diff[0] != "0":  # equal would indicate 0 days
            return " ".join(
                diff.split(" ")[:-1]
            )  # k days, only look at days ignore hours
        return (
            diff.split(" ")[-1:][0].split(":")[0] + " hours"
        )  # k hours, here days is 0 so not mentioned

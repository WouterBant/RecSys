...
class P5(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs,
        ...
        #print('case1', return_hidden_state)
        if return_hidden_state:
            return sequence_output

        lm_logits = self.lm_head(sequence_output) # score before softmax

        prob = torch.softmax(lm_logits.view(-1, lm_logits.size(-1)), dim=-1)
        ...
        if labels is not None:
            ...
            if reduce_loss:
                ...
                loss_fct = CrossEntropyLoss(ignore_index = -100, reduction='none') # for each sample, take softmax to get the prob, and then compute average loss over all samples
